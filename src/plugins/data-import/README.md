...
**Note** the `dataset`, `person` and `disease_gene` content-type need additional setup. `dataset` has a `dataset_id` unique field that is automatically generated from the dataset `uid` (also automatically generated) and the related `study`'s `id` (if any or `null`) This is to support datasets having same name but different study, instead of enforcing a unique name across all datasets. A `dataset` update through the plugin should be through a file that specifies the `dataset`'s `name` and related `study_id` (even if the study will be created in the import). `person` contains `first_name` and `last_name` fields which by themselves are not useful as unique identifiers. For that reason the `person` model uses a `full_name` field which is non-configurable through the admin UI and generated from the `first_name` and `last_name` fields in its lifecycle hooks. This `full_name` field is then used as the `uid` field's target. `disease_gene` contains `gene_id` and `disease_id` fields which are automatically used to set a `disease_gene` field which is the `uid` field's target.
